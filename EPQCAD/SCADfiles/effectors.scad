...
module electromagnet_sphere(handle_height = 25, magnet_diameter = 20, beam_diameter = 5){
  //surface_height = 15;
  ...
  union(){    
    difference(){
      ...
      difference(){
        ...
        cylinder(h=handle_height+beam_diameter-(wire_height-wirehole_radius), r=case_radius, center=false);
        ...
        cylinder(h=handle_height+beam_diameter+(magnet_height-wire_height)+0.02, r=magnet_radius, center=false);

        translate([0,0,wire_height-wirehole_radius+(handle_height+beam_diameter+(magnet_height-wire_height))/2+0.01])
        cube([magnet_radius*2,case_radius*2,handle_height+beam_diameter+(magnet_height-wire_height)], center = true);
      }
      //wirehole
      ...
      rotate([0,90,0])
      cylinder(h=case_radius*3, r=beam_diameter/2, center=true);
    }    
    difference(){
